...
/**
 * Support for creating partial URI converters, typically converting HTTP or HTTPS URIs to local file URIs. Typically a
 * PartialUriConverter is created from a SimpleCatalog. This is also desirable, because catalogs are clear and precise URI mappings.
 * Yet not every PartialUriConverter can be created from a SimpleCatalog, for example because an URI may map to the first of a list of
...
object PartialUriConverters {
  ...
  val identity: PartialUriConverter = { uri: URI => Some(uri) }
   ...
   */
  def fromCatalogs(catalogs: Seq[SimpleCatalog], acceptTargetUri: URI => Boolean): PartialUriConverter = {
